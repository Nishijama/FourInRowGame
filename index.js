document.addEventListener("DOMContentLoaded", () => {
  ...
  menuForm.addEventListener("submit", event => {
    ...
    const displayCurrentPlayer = document.querySelector(
    ...
    function timeOver() {
      if (gameState === "playing") {
        ...
        else popUp.children[1].innerText = "Player 1 wins! :)";
    ...
    for (let i = 0; i < squares.length; i++) {
      squares[i].onclick = () => {
        if (
          squares[i + columnNumber].classList.contains("taken") &&
          !(
            squares[i].classList.contains("player_one") ||
            squares[i].classList.contains("player_two")
          )
        ) {
          ...
          popUp.children[1].innerText =
            "Your circle cannot float in the air :)";
      ...
      for (let i = 0; i < squares.length - columnNumber; i++) {
        ...
        if ((i % columnNumber) + 4 <= columnNumber) {
          ...
          if (
            ...
            squares[i + (columnNumber + 1) * 2].classList.contains(
              "player_one"
            ) &&
        ...
        if (numberOfTakenFields == columnNumber * rowNumber) winner = 3;
